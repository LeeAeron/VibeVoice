...
def main():
    args = parse_args()
    # Determine whether the model is Q8-quantized
    is_q8_model = any(q in args.model_path.lower() for q in ["q8", "quantized", "fabiosarracino"])
    ...
    # Определяем, является ли модель Q8-квантизированной
    is_q8_model = any(q in args.model_path.lower() for q in ["q8", "quantized", "fabiosarracino"])

    # Загружаем модель
    try:
        if is_q8_model:
            print("🎯 Detected Q8 quantized model")
            print("📊 Model size: ~11.6 GB (vs 18.7 GB original)")
            print("💾 VRAM usage: ~12 GB (vs 20 GB original)")
            ...
            # Определяем dtype и attention impl
            if args.device == "mps":
                load_dtype = torch.float32
                attn_impl_primary = "sdpa"
            elif args.device == "cuda":
        ...
        if not is_q8_model and attn_impl_primary == "flash_attention_2":
            print("⚠️ Falling back to SDPA attention")
            model = VibeVoiceForConditionalGenerationInference.from_pretrained(
                args.model_path,
                torch_dtype=load_dtype,
                device_map=(args.device if args.device in ("cuda", "cpu") else None),
    ...
    if is_q8_model:
        ...
        print("🎵 Audio quality: Identical to original")
        print("⚡ Optimized for 12–16 GB VRAM GPUs")
